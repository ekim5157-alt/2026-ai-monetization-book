...
각 에이전트는 작업 시작 전 해당 단계의 프롬프트 파일을 GitHub에서 직접 읽는다. 읽지 않은 상태로 작업을 시작하지 않는다.

| 단계 | 필수 프롬프트 |
|---|---|
| 총괄 편집 | `prompts/00_총괄_편집실.md` |
| business (사업성 분석) | `prompts/01_사업성_분석.md` |
| research (공식자료 조사) | `prompts/02_공식자료_조사.md` |
| draft (초보자 실행 원고) | `prompts/03_초보자_실행_원고.md` |

- 프롬프트 파일에 적힌 필수 입력·조사 원칙·출력 구조·저장 규칙을 모두 따른다.
- 프롬프트 파일의 지침과 AGENTS.md가 충돌할 경우 사용자에게 알리고 판단을 기다린다.
